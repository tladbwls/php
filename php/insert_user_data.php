...
//     $sql = "SELECT * FROM spl_user"; 
    ...
    // 3. 데이터 조회 필터링 :SELECT [field name] FROM [table name] WHERE [field name] = [field value];
        ...
        // 4. 데이터 정렬 : SELECT [field name] FROM [table name] ORDER BY [field nam] DESC (역순) or ASC (정순)
        ...
        // $sql = "UPDATE spl_user SET user_name = '심유진' WHERE user_idx = 1";

        //7. 데이터 삭제 (delete) : DELETE FROM [table name] WHERE [field name] = [field value];
        $sql = "DELETE FROM spl_user WHERE user_idx = 1 and user_name = '심유진'"
